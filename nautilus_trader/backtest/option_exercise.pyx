...
handling both cash-settled and physically-settled options:
...
The module automatically detects settlement type based on the underlying instrument:
- IndexInstrument -> Cash settlement
- Other instruments -> Physical settlement
...
cdef class OptionExerciseModule(SimulationModule):
    """
    Simulation module for automatic option exercise at expiry.

    This module monitors option positions during backtesting and automatically
    exercises in-the-money options at expiry. The settlement behavior depends on
    the underlying instrument type:

    **Cash Settlement (Index Options)**:
    - Options are closed at their intrinsic value
    - No underlying position is created
    - Cash payment equals: max(0, underlying_price - strike) for calls,
      max(0, strike - underlying_price) for puts

    **Physical Settlement (Equity Options)**:
    - Options are closed and underlying positions are created
    - Underlying position created at strike price
    - Position quantity = option_quantity * multiplier

    The module automatically detects settlement type by checking if the underlying
    instrument is an IndexInstrument (cash settlement) or other instrument types
    (physical settlement).

    Parameters
    ----------
    config : OptionExerciseConfig
        The configuration for the option exercise module.
    ...
    cpdef void register_venue(self, SimulatedExchange exchange):
        ...
        if hasattr(self, "_msgbus") and self._msgbus:
    ...
    def on_position_event(self, event) -> None:
        ...
        if isinstance(event, PositionOpened):
            ...
            if expiry_ns not in self.expiry_timers:
                ...
                # Set timer for option expiry using the actor's clock
    ...
    def _process_option_expiry(self, option: OptionContract | CryptoOption, ts_now: int) -> None:
        ...
        for position in positions:
            if position.side == PositionSide.LONG:
                # Long positions: evaluate for exercise
                if self._should_exercise(option, underlying_price, "exercise"):
                    self._exercise_position(option, position, underlying_price, ts_now)
                # If OTM, long option expires worthless (no action needed)
            else:
                # Short positions: evaluate for assignment
                # Assignment happens when the option is ITM (long holder would exercise)
                is_itm, _ = self._is_option_itm(option, underlying_price)

                if is_itm:
                    # Option is ITM - short position gets assigned
                    if self._should_exercise(option, underlying_price, "assignment"):
                        self._assign_position(option, position, underlying_price, ts_now)
                # If OTM, short option expires worthless and position is closed at zero value
                else:
                    self._log.debug(
    ...
    def _should_exercise(self, option: OptionContract | CryptoOption, underlying_price: Price, action: str) -> bool:
        """
        Common logic to determine if option should be exercised/assigned.
        """
        is_itm, intrinsic_value = self._is_option_itm(option, underlying_price)

        # Get exercise threshold based on underlying instrument's tick size
        underlying_instrument = self._get_underlying_instrument(option)
        if underlying_instrument is None:
            # Fallback to option's price increment if underlying not found
            exercise_threshold = option.price_increment.as_double()
        else:
            # Use underlying's tick size as the threshold
            exercise_threshold = underlying_instrument.price_increment.as_double()

        if not is_itm or intrinsic_value < exercise_threshold:
            self._log.debug(
                f"Skipping {action} of {option.id}: OTM or below threshold "
                f"(intrinsic: {intrinsic_value:.4f}, threshold: {exercise_threshold})",
            )
            return False
        return True
    ...
    def _exercise_position(
        ...
        """Execute the option exercise, creating underlying position and closing option."""
        self._process_option_settlement(option, position, underlying_price, ts_now, "exercise")
    ...
    def _assign_position(
        ...
        """Execute the option assignment, creating underlying position and closing option."""
        self._process_option_settlement(option, position, underlying_price, ts_now, "assignment")

    def _process_option_settlement(
        self,
        option: OptionContract | CryptoOption,
        position: Position,
        underlying_price: Price,
        ...
        """
        Process option settlement (exercise or assignment) with unified logic.

        Automatically determines settlement type based on underlying instrument:
        ...
        ----------
        option : OptionContract | CryptoOption
            The option contract being settled.
        position : Position
            The option position being settled.
        underlying_price : Price
            Current price of the underlying instrument.
        ts_now : int
            Current timestamp in nanoseconds.
        action : str
            Settlement action type ("exercise" or "assignment").
        """
        underlying_instrument = self._get_underlying_instrument(option)
        is_cash_settled = isinstance(underlying_instrument, IndexInstrument)
        settlement_type = "cash" if is_cash_settled else "physical"

        action_verb = "Exercising" if action == "exercise" else "Assigning"
        self._log.debug(
            f"{action_verb} {option.id}: {position.side} {position.quantity} @ strike {option.strike_price} "
            f"(underlying: {underlying_price}, settlement: {settlement_type})",
        )

        if is_cash_settled:
            # Cash settlement: close option at intrinsic value
            self._generate_cash_settlement_events(option, position, underlying_price, ts_now, action)
        else:
            ...
            if underlying_instrument is None:
                self._log.error(f"Cannot {action} {option.id}: underlying instrument not found")
                return

            self._generate_physical_settlement_events(
                option, position, underlying_instrument, underlying_quantity,
                underlying_side, underlying_price, ts_now, action
            )
    ...
    def _get_underlying_price(self, option: OptionContract | CryptoOption) -> Price | None:
        ...
        # Get latest quote or trade from cache
        if self.cache:
            # Try to get latest quote first (more accurate for exercise decisions)
            quote = self.cache.quote_tick(underlying_instrument.id)
            if quote is not None:
                # Use mid price for exercise decisions
                mid_price_value = (quote.bid_price.as_double() + quote.ask_price.as_double()) / 2.0
                return Price(mid_price_value, quote.bid_price.precision)

            # Fallback to latest trade if no quote available
            trade = self.cache.trade_tick(underlying_instrument.id)
    ...
    def _cleanup_timer_if_no_positions(self, expiry_ns: int) -> None:
        ...
        if not has_positions:
            ...
            if hasattr(self.clock, "cancel_timer"):
                self.clock.cancel_timer(timer_name)
            elif hasattr(self.clock, "cancel_time_alert"):
    ...
    cpdef Instrument _get_underlying_instrument(self, object option):
        ...
        # Find underlying instrument in cache
        for instrument in self.cache.instruments():
            if str(instrument.id).startswith(str(underlying_symbol)):
                return instrument
    ...
    def _generate_cash_settlement_events(
        ...
        ----------
        option : OptionContract | CryptoOption
            The option contract being cash settled.
        position : Position
            The option position being settled.
        underlying_price : Price
            Current price of the underlying instrument.
        ...
        action_past = "exercised" if action == "exercise" else "assigned"
        self._log.debug(
            f"Cash settlement complete: {action_past} {option.id} position @ {intrinsic_value} "
    ...
    def _generate_physical_settlement_events(
        ...
        ----------
        option : OptionContract | CryptoOption
            The option contract being physically settled.
        position : Position
            The option position being settled.
        underlying_instrument : Instrument
            The underlying instrument for position creation.
        underlying_quantity : Quantity
            Quantity of underlying position to create.
        underlying_side : PositionSide
            Side of the underlying position (LONG or SHORT).
        underlying_price : Price
            Current price of the underlying instrument.
        ts_now : int
            Current timestamp in nanoseconds.
        ...
        option_close_fill = self._create_option_fill(
            option, position, f"{action_upper}_CLOSE_{settlement_id}",
            f"{action_upper}_{settlement_id}", ts_now, True
        )
        ...
        underlying_open_fill = self._create_underlying_fill(
            ...
            settlement_price, f"{action_upper}_OPEN_{settlement_id}", f"{action_upper}_{settlement_id}", ts_now
        )

        self._send_events([option_close_fill, underlying_open_fill])
        action_past = "exercised" if action == "exercise" else "assigned"
        self._log.debug(
            f"Physical settlement complete: {action_past} {option.id} position, "
    ...
    def _calculate_settlement_price(self, option, underlying_price: Price) -> Price:
        ...
        -------
        Price
            Settlement price for the option.
        """
        underlying_instrument = self._get_underlying_instrument(option)
        is_cash_settled = isinstance(underlying_instrument, IndexInstrument)
    ...
    def _send_events(self, events: list) -> None:
        ...
        if self.exchange and hasattr(self.exchange, "msgbus"):
            for event in events:
                self.exchange.msgbus.send(endpoint="ExecEngine.process", msg=event)

    # Testing helper methods to work around Cython limitations
    def _test_get_underlying_instrument(self, option):
        """Test wrapper for _get_underlying_instrument that accepts any object."""
        return self._get_underlying_instrument(option)
